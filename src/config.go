...
func Init() (err error) {
	...
	System.FFmpeg.DefaultOptions = "-hide_banner -loglevel error -analyzeduration 1000000 -probesize 1000000 -i [URL] -map 0:v -map 0:a:0 -c:v copy -c:a aac -b:a 192k -ac 2 -c:s copy -f mpegts -mpegts_flags +resend_headers -fflags +genpts+discardcorrupt pipe:1"
	...
	System.WEB.Menu = []string{"playlist", "xmltv", "filter", "mapping", "users", "log", "settings", "logout"}
